# DSPy・ReAct・LLM Judge・GEPAを0から理解する初心者リファレンス

この資料は、PythonとDSPyを初めて見る人向けの「辞書兼教科書」です。  
実際に複雑な問題をコピーして動かすときは、同梱の次の資料を先に使ってください。

```text
dspy_gepa_complex_experiment_copy_paste.md
```

このリファレンスでは、次を0から説明します。

- Pythonの`import`、変数、文字列、リスト、辞書、関数、クラス
- DSPyの`Signature`、`Module`、`Prediction`、`Example`
- ReActと`trajectory`
- LLM Judgeとmetric
- GEPAのscoreとfeedback
...
これまでの試行では、1ツール問題は最適化前から正しく、GEPAの改善は確認できませんでした。2ツール問題では、「1.2 km、800 m、1500 mの平均」の1問で、`calculate_expression`だけだった行動が`convert_units → calculate_expression`へ変わり、スコアが約1ポイント上がりました。ただし、期待した`convert_units → analyze_numbers`には達していないため、部分的改善です。

この経験から、追加実験では次を導入します。

1. 2〜4ツールが必要な問題
2. trainset・valset・testsetの分離
3. 期待ツール順序の明示
4. 総合点とは別の「ツール順序完全一致率」
5. 1ツール性能を守るcontrolset

詳しい追加実験コードは、前述のコピペ実行ガイドにあります。
...
# 追加章：今回の複雑問題実験へ進むときの読み替え

このリファレンスの基本コードでは、比較的単純な問題と、LLM Judge中心の評価を説明しています。追加実験では、次の点を強化しています。

| 基本版 | 複雑問題版 |
|---|---|
| 1〜2ツール中心 | 2〜4ツール |
| Judgeが適切さを判断 | 期待ツール列も明示 |
| trainsetとtestset中心 | train/val/test/control |
| 総合点中心 | 総合点＋完全一致率 |
| 代替ツールにも寛容 | 制御実験として期待順序を評価 |

## なぜ2つの資料に分けたか

初心者が一度にすべて読むと、次の2つが混ざりやすいためです。

- Python/DSPyの仕組みを理解すること
- 実験コードを正しい順番で動かすこと

実習中はコピペ実行ガイド、時間のあるときはこのリファレンス、という使い分けを推奨します。
...
順番を保った複数の値です。同じツールを2回使う場合は、同じ文字列を2回入れます。
...
result = {
    "score": 0.82,
    "feedback": "統計ツールを使う必要があります",
}
```

名前と値の組をまとめます。`result["score"]`で0.82を取り出せます。

### 真偽値

```python
sequence_exact = expected_tools == actual_tools
```

2つのリストが完全に同じなら`True`、異なれば`False`です。

### 平均

```python
from statistics import mean
average = mean([0.8, 0.7, 0.9])
```
...
実験結果を後からExcelで確認できるよう保存します。`with`を使うと、処理後にファイルを自動で閉じます。

## 実験の読み方

GEPA後に点数が上がったときも、次を分けて見ます。
...
点数1つだけで「成功」と結論づけないことが、今回の研究テーマでは重要です。
